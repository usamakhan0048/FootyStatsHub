  ...
  const fetchTopScorers = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `https://v3.football.api-sports.io/players/topscorers?league=${selectedLeague}&season=${selectedSeason}`,
        {
          method: 'GET',
          headers: myHeaders,
          redirect: 'follow'
                  ...
                  <table className="w-full">
                    <thead>
                      <tr className="border-b border-purple-400/20">
                        ...
                        <th className="px-6 py-4 text-right text-md font-semibold text-purple-100"> Goals</th>
                        <th className="px-6 py-4 text-right text-md font-semibold text-purple-100">Assists</th>
                      ...
                      {topScorers.map(({ player, statistics }, index) => (
                        <tr key={player.id} className="border-b border-purple-400/10 hover:bg-white/5 transition-colors duration-200">
                          ...
                          <td className="px-6 py-4 text-right">
                            <span className="text-md text-white">{statistics[0].goals.total}</span>
                          </td>
                          <td className="px-6 py-4 text-right">
                            <span className="text-md text-white">{statistics[0].goals.assists}</span>
                          </td>
                          <td className="px-6 py-4 text-right">
