  ...
  const fetchTopRedCards = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `https://v3.football.api-sports.io/players/topredcards?league=${selectedLeague}&season=${selectedSeason}`,
        {
          method: 'GET',
          headers: myHeaders,
          redirect: 'follow'
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="space-y-8">
            <div className="text-center">
              <h1 className="text-4xl font-bold text-white mb-2">Top Red Cards</h1>
            </div>
                  ...
                  <table className="w-full">
                    <thead>
                      <tr className="border-b border-purple-400/20">
                        ...
                        <th className="px-6 py-4 text-right text-md font-semibold text-purple-100">Red Cards</th>
                    ...
                    <tbody>
                      {topRedCards.map(({ player, statistics }, index) => (
                        <tr key={player.id} className="border-b border-purple-400/10 hover:bg-white/5 transition-colors duration-200">
                          ...
                          <td className="px-6 py-4 text-right">
                            <span className="text-md text-white">{statistics[0].cards.red}</span>
                          </td>
                          <td className="px-6 py-4 text-right">
